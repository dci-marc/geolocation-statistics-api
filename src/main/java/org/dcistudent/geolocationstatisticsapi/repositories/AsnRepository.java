...
public interface AsnRepository extends JpaRepository<Asn, Long> {
  ...
  List<Asn> findAsByNumber(@NotNull @Param("asNumber") Long asNumber);

  @Query("SELECT a " +
      "FROM Asn AS a " +
      "WHERE a.id = :id")
  List<Asn> findAsById(@NotNull @Param("id") Long id);

  @Query("SELECT a " +
      "FROM Asn AS a " +
      "WHERE a.asOrg LIKE CONCAT('%', :name, '%')")
  List<Asn> findAsByName(@NotNull @Param("name") String name);
}
